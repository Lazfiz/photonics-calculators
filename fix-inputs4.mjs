...
for (const file of getFiles()) {
  ...
  // Pattern: <label className="block text-sm font-medium mb-2">LABEL</label>
  //          <input type="number" value={x} onChange={(e) => setX(Number(e.target.value))} ... />
  // Replace both with a single <InputSlider />
  const regex = /<label className="block text-sm font-medium mb-2">([\s\S]*?)<\/label>\s*\n?\s*<input type="(?:number|range)"([\s\S]*?)\s*\/>/g;
  ...
  if (regex.test(src)) {
    ...
    src = src.replace(regex, (match, labelText, attrs) => {
      ...
      // Handle both e => setX(+e.target.value) and (e) => setX(Number(e.target.value))
