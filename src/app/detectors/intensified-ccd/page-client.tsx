...
export default function IntensifiedCCDPage() {
  ...
  const phosphors = { p43: { eff: 0.15, decay: 1.2, label: "P43 (Gd₂O₂S:Tb)" }, p46: { eff: 0.25, decay: 0.0001, label: "P46 (YAG:Ce)" }, p20: { eff: 0.12, decay: 3.0, label: "P20 (ZnCdS:Ag)" } };

  const pc = photocathodes[photocathodeType];
  const ph = phosphors[phosphorType];
  const detectedElectrons = signalPhotons * pc.qe;
  const electronGain = mcpGain * ph.eff * fiberCoupling * ccdQE;
  ...
  const gainChart = useMemo(() => {
    ...
    return [
      { x: gains, y: gains.map(g => detectedElectrons * g * ph.eff * fiberCoupling * ccdQE), type: "scatter", mode: "lines", name: "Output signal", line: { color: "#60a5fa" }, yaxis: "y" },
      { x: gains, y: gains.map(g => Math.sqrt(detectedElectrons * g ** 2 * ph.eff ** 2 * fiberCoupling ** 2 * ccdQE ** 2 * 2 + ccdReadNoise ** 2)), type: "scatter", mode: "lines", name: "Total noise", line: { color: "#f87171" }, yaxis: "y" },
      { x: gains, y: gains.map(g => { const s = detectedElectrons * g * ph.eff * fiberCoupling * ccdQE; const n = Math.sqrt(detectedElectrons * g ** 2 * ph.eff ** 2 * fiberCoupling ** 2 * ccdQE ** 2 * 2 + ccdReadNoise ** 2); return s / n; }), type: "scatter", mode: "lines", name: "SNR", line: { color: "#34d399" }, yaxis: "y2" },
    ];
  }, [detectedElectrons, ph.eff, fiberCoupling, ccdQE, ccdReadNoise]);
  ...
  return (
    <CalculatorShell backHref="/detectors" backLabel="Detectors" title="Intensified CCD (ICCD)" description="Photocathode → MCP → phosphor → CCD gain chain with gating and noise analysis." maxWidthClassName="max-w-5xl">
      ...
      <div className="bg-gray-900 rounded-lg p-4 mb-6 text-sm text-gray-300 font-mono space-y-1"><p>Gain = η_pc × G_MCP × η_phos × η_fiber × η_CCD</p><p>σ²_total = S·G²·F + σ_read², F_MCP ≈ 2</p><p>Phosphor decay: {ph.label} → {ph.decay >= 0.01 ? `${(ph.decay).toFixed(1)} ms` : `${(ph.decay * 1e6).toFixed(0)} ns`}</p></div>
