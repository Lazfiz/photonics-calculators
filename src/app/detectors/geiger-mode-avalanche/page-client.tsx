...
export default function GeigerModeAPDPage() {
  ...
  const results = useMemo(() => {
    ...
    const effectiveDCR = effectiveOverbias <= 0 ? 0 : darkCountRate * dcrTempFactor;
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      { x: temps, y: temps.map(t => { const eo = overbias - tempCoeff * 1e-3 * (t - 25); return eo <= 0 ? 0 : darkCountRate * Math.pow(2, (t - 25) / 10); }), type: "scatter", mode: "lines", name: "DCR", line: { color: "#a78bfa" }, yaxis: "y3" },
    ];
  ...
  return (
    <CalculatorShell backHref="/detectors" backLabel="Detectors" title="Geiger-Mode APD" description="SPAD: breakdown voltage, overbias, temperature effects, PDE, and dark count rate.">
      ...
      <div className="bg-gray-900 rounded-lg p-4 mb-6 text-sm text-gray-300 font-mono space-y-1"><p>ΔVbr = β·(T−Tref)</p><p>PDE ≈ 0.15·V_over(eff), PDE=0 at breakdown</p><p>R_max = 1/τ_dead, DCR(T) = DCR₀·2^((T−25)/10)</p></div>
