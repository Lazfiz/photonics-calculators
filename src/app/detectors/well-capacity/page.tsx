...
export default function WellCapacityPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Well Capacity & Dynamic Range</h1>
      <p className="text-gray-400 mb-8">DR = 20·log₁₀(N<sub>well</sub>/σ<sub>read</sub>). C = N<sub>well</sub>·q/V<sub>swing</sub>. Larger wells → more DR but slower charge transfer.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Pixel size (μm)</span>
          <input type="number" value={pixelSize} onChange={e => setPixelSize(+e.target.value)} step="0.5" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Voltage swing (V)</span>
          <input type="number" value={voltageSwing} onChange={e => setVoltageSwing(+e.target.value)} step="0.1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
