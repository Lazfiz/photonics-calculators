...
export default function ThermalDnDtPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Thermo-Optic Coefficient (dn/dT)</h1>
      <p className="text-gray-400 mb-8">Temperature-dependent refractive index change. Positive dn/dT means n increases with temperature.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block">
          <span className="text-gray-300 text-sm">Base Temperature (°C)</span>
          <input type="number" value={baseTemp} onChange={e => setBaseTemp(+e.target.value)}
            className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" />
        </label>
        <label className="block">
          <span className="text-gray-300 text-sm">Temperature Change ΔT (°C)</span>
          <input type="number" value={deltaT} onChange={e => setDeltaT(+e.target.value)}
            className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" />
        </label>
        <label className="block">
          <span className="text-gray-300 text-sm">Base Refractive Index n₀</span>
