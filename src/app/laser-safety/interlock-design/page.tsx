...
export default function InterlockDesignPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Interlock Time Calculation</h1>
      <p className="text-gray-400 mb-8">Calculates required interlock/shutter response time based on laser hazard level. IEC 60825-1 and ANSI Z136.1 require interlocks to terminate emission before exposure exceeds MPE.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-8">
        ...
        <label className="block">
          <span className="text-gray-300 text-sm">Shutter Response (ms)</span>
          <input type="number" value={shutterTime} onChange={e => setShutterTime(+e.target.value)} min={0.1} step="0.1"
            className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" />
        </label>
        <label className="block">
          <span className="text-gray-300 text-sm">Human Reaction Time (ms)</span>
          <input type="number" value={humanResponse} onChange={e => setHumanResponse(+e.target.value)} min={50}
            className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" />
        </label>
        <label className="block">
          <span className="text-gray-300 text-sm">Access Distance (cm)</span>
