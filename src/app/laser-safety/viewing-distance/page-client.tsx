    ...
    >
      ...
      {result.status === "supported" ? (
        <>
          ...
          <SimpleLineChart title="Irradiance vs distance" xLabel="Distance (m)" yLabel="Irradiance (W/cm²)" xScale="log" yScale="log" series={[{ name: "Irradiance", color: "#60a5fa", points: chartData[0]?.x.map((x: number, i: number) => ({ x, y: chartData[0].y[i] })) ?? [] }, { name: "Target irradiance", color: "#f87171", dashed: true, points: chartData[1]?.x.map((x: number, i: number) => ({ x, y: chartData[1].y[i] })) ?? [] }]} />
        </>
      ) : (
