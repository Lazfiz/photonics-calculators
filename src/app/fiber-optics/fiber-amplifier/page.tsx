...
export default function FiberAmplifierCalculator() {
  ...
  const [fiberLength, setFiberLength] = useState<number>(10); // m
  ...
  const [erbiumConc, setErbiumConc] = useState<number>(1e24); // ions/m³
  const [coreRadius, setCoreRadius] = useState<number>(2.2); // µm
  const [overlap, setOverlap] = useState<number>(0.8); // Γ factor
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">Fiber Amplifier Calculator</h1>
        <p className="text-gray-400 mb-8">EDFA / YDFA design — gain spectrum, saturation, noise figure, pump requirements</p>
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-8">
          <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
            <Plot data={gainSpectrum} layout={layout1} config={{ responsive: true }} className="w-full" />
          </div>
          <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
            <Plot data={gainVsPump} layout={layout2} config={{ responsive: true }} className="w-full" />
          </div>
        </div>
      </div>
    </div>
