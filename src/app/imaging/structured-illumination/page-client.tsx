      ...
      <div className="grid gap-6 md:grid-cols-2 mb-8">
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-4">
          ...
          <div>
            ...
            <input type="number" step={0.05} min={0.1} max={1} value={modulationDepth} onChange={e => setModulationDepth(+e.target.value)}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Pattern Orders (2×: 2, 3×: 3)</label>
            <input type="number" step={1} min={1} max={5} value={patternOrders} onChange={e => setPatternOrders(+e.target.value)}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white" />
          </div>
        </div>
