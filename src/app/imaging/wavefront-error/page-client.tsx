...
export default function WavefrontErrorPage() {
  ...
  const [rmsError, setRmsError] = useURLState("rmsError", 0.25);
  const [numZernike, setNumZernike] = useURLState("numZernike", 6);

  // Wavefront error in waves and nm
  const wfErrorWaves = rmsError;
  const wfErrorNm = rmsError * wavelengthNm;
  const wfErrorPv = rmsError * 3.55 * wavelengthNm; // PV ≈ 3.55×RMS for random aberrations

  // Marechal criterion: RMS < λ/14
  const marechalLimit = 1 / 14;
  const isDiffractionLimited = rmsError < marechalLimit;

  // Strehl ratio approximation
  const strehl = Math.exp(-((2 * Math.PI * rmsError) ** 2));
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      { x: [rmsError], y: [strehl], type: "scatter", mode: "markers", name: "Current", marker: { color: "#f87171", size: 12 } },
  ...
  const zernikeData = useMemo(() => {
    ...
    return [
      { x: terms, y: terms.map(() => rmsError / Math.sqrt(numZernike)), type: "bar", name: "RMS per term", marker: { color: "#a78bfa" } },
    ];
  }, [rmsError, numZernike]);
    ...
    <CalculatorShell backHref="/imaging" backLabel="Imaging" title="Wavefront Error Analysis" description="Analyze wavefront error in waves RMS, compute Strehl ratio, and check diffraction-limited condition.">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-blue-400">{rmsError.toFixed(3)} λ</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-6">
        ...
        <div className="space-y-2 text-sm text-gray-300 font-mono">
          <p>S ≈ exp(−(2π·σ_rms)²) — Maréchal approximation</p>
