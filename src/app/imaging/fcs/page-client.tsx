...
export default function FCSPage() {
  ...
  const [tauD, setTauD] = useURLState("tauD", 0.05); // ms diffusion time
  const [brightness, setBrightness] = useURLState("brightness", 30000); // Hz per molecule
  const [concentration, setConcentration] = useURLState("concentration", 50); // nM
  const [tripletFrac, setTripletFrac] = useURLState("tripletFrac", 0.1);
  const [tripletTau, setTripletTau] = useURLState("tripletTau", 0.005); // ms
