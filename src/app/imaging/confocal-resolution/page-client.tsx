...
export default function ConfocalResolutionPage() {
  ...
  // Lateral resolution (confocal ~0.7× widefield for finite pinhole)
  const widefieldLateral = 0.61 * wavelength / na;
  const confocalLateral = 0.7 * wavelength / na;

  // Axial resolution
  const widefieldAxial = 2 * refractiveIndex * wavelength / (na * na);
  const confocalAxial = 1.4 * refractiveIndex * wavelength / (na * na);

  // Pinhole: 1 AU = Airy disk diameter at image plane = 1.22λ/NA × M
  const airyDiameterUm = 1.22 * wavelength / (na * 1000); // µm, object space
  const pinholeUm = pinholeAU * airyDiameterUm * magnification; // µm, image plane
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      { x: nas, y: nas.map(n => 0.7 * wavelength / n), type: "scatter", mode: "lines", name: "Confocal Lateral", line: { color: "#34d399" } },
