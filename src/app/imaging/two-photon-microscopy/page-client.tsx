...
export default function TwoPhotonMicroscopyPage() {
  ...
  const [exposureDepth, setExposureDepth] = useURLState("exposureDepth", 200);
  const [refractiveIndex, setRefractiveIndex] = useURLState("refractiveIndex", 1.33);
  const [scatteringCoeff, setScatteringCoeff] = useURLState("scatteringCoeff", 100);
  ...
  return (
    <CalculatorShell backHref="/imaging" backLabel="Imaging" title="Two-Photon Microscopy Calculator" description="Calculate resolution, excitation volume, peak intensity, and depth penetration for two-photon fluorescence microscopy.">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <ValidatedNumberInput label="Avg Power (mW)" value={avgPower} onChange={setAvgPower} min={1} max={200} />
      </div>
