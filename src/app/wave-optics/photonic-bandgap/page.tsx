...
export default function PhotonicBandgapPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Photonic Bandgap</h1>
      <p className="text-gray-400 mb-8">1D photonic crystal band structure and reflectivity.</p>

      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">Lattice Constant a (nm)</span>
          <input type="number" value={latticeConst} onChange={e => setLatticeConst(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">High Index n₁</span>
          <input type="number" value={nHigh} onChange={e => setNHigh(+e.target.value)} step="any" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Low Index n₂</span>
          <input type="number" value={nLow} onChange={e => setNLow(+e.target.value)} step="any" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Fill Fraction</span>
          <input type="number" value={fillFraction} onChange={e => setFillFraction(+e.target.value)} step="any" min={0} max={1} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Number of Periods</span>
          <input type="number" value={numPeriods} onChange={e => setNumPeriods(+e.target.value)} min={1} max={100} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
