...
export default function DiffractionIntegralPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Diffraction Integral Calculator</h1>
      <p className="text-gray-400 mb-8">Fresnel/Kirchhoff diffraction patterns.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-4">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Aperture</span>
          <select value={apertureType} onChange={e => setApertureType(e.target.value as any)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white">
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">Aperture size (µm)</span>
          <input type="number" value={apertureSize} onChange={e => setApertureSize(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Propagation distance (mm)</span>
          <input type="number" value={propDist} onChange={e => setPropDist(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
      ...
      {apertureType === "circular" && (calc as any).z2d && (
        <div className="bg-gray-900 rounded-lg p-4">
          <Plot data={[{ z: (calc as any).z2d, x: (calc as any).xs, y: (calc as any).xs, type: "heatmap" as const, colorscale: "Inferno" }]} layout={{
            paper_bgcolor: "transparent", plot_bgcolor: "transparent",
