...
export default function RingCavityPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Ring Resonator Design</h1>
      <p className="text-gray-400 mb-8">Ring cavity stability, modes, and spectral analysis.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-4">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Round-trip length (mm)</span>
          <input type="number" value={roundTripLength} onChange={e => setRoundTripLength(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 mb-4">
        <label className="block"><span className="text-gray-300 text-sm">Mirror ROC (mm)</span>
          <input type="number" value={mirrorR} onChange={e => setMirrorR(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Number of mirrors</span>
          <input type="number" value={numMirrors} onChange={e => setNumMirrors(Math.max(3, +e.target.value))} min={3} max={6} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">Output coupler R</span>
          <input type="number" value={outputCouplerR} onChange={e => setOutputCouplerR(+e.target.value)} step="0.01" min={0} max={0.999} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Intracavity loss (per round trip)</span>
          <input type="number" value={intracavityLoss} onChange={e => setIntracavityLoss(+e.target.value)} step="0.001" min={0} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
