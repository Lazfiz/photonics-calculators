...
export default function BesselBeamPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Bessel Beam Calculator</h1>
      <p className="text-gray-400 mb-8">Non-diffracting beam profiles and propagation.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Cone angle α (rad)</span>
          <input type="number" value={alpha} onChange={e => setAlpha(+e.target.value)} step="any" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Max radius (µm)</span>
          <input type="number" value={maxR} onChange={e => setMaxR(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
      ...
      <div className="bg-gray-900 rounded-lg p-4">
        <Plot data={[{ z: calc.z, x: calc.xs, y: calc.xs, type: "heatmap" as const, colorscale: "Inferno", name: "J₀ 2D" }]} layout={{
          paper_bgcolor: "transparent", plot_bgcolor: "transparent",
