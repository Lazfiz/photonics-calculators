...
export default function ModeMatchingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Mode Matching</h1>
      <p className="text-gray-400 mb-8">Find the optimal lens for coupling one Gaussian beam mode into another.</p>
...
Single thin lens mode matcher: find f and s
  s = distance from input waist to lens`}</pre>
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Input waist w₁ (µm)</span>
          <input type="number" value={w1} onChange={e => setW1(+e.target.value)} step="any" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Target waist w₂ (µm)</span>
          <input type="number" value={w2} onChange={e => setW2(+e.target.value)} step="any" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>

      <label className="block mb-6"><span className="text-gray-300 text-sm">Distance between waists d: {d} mm</span>
