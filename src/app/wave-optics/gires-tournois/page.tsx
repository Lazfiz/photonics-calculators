...
export default function GiresTournoisPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Gires-Tournois Interferometer</h1>
      <p className="text-gray-400 mb-8">Dispersion control via a GTI — constant reflectivity with tunable group delay dispersion.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">Front Surface R₁</span>
          <input type="number" value={reflectivity} onChange={e => setReflectivity(+e.target.value)} step="0.01" min="0.01" max="0.99" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        ...
        <label className="block"><span className="text-gray-300 text-sm">Plot Bandwidth (nm)</span>
      ...
      <div className="bg-gray-900 rounded-lg p-4">
        ...
        <Plot data={gdData} layout={layout2} config={{ responsive: true, displayModeBar: false }} />
      </div>
    </div>
  );
}
