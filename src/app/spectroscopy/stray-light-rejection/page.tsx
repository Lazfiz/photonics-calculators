...
export default function StrayLightRejectionPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <p className="text-gray-400 mb-8">Impact of stray light on photometric accuracy. Critical for high-absorbance measurements.</p>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">Stray Light Ratio</span>
          <input type="number" value={strayLightRatio} onChange={e => setStrayLightRatio(Math.max(0, +e.target.value))} min={0} step={0.0001} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">True Absorbance</span>
          <input type="number" value={absorbance} onChange={e => setAbsorbance(+e.target.value)} min={0} step={0.1} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Grating Lines (l/mm)</span>
          <input type="number" value={gratingLines} onChange={e => setGratingLines(+e.target.value)} min={50} step={100} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
