...
export default function GratingEfficiencyPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Grating Efficiency Calculator</h1>
      <p className="text-gray-400 mb-8">Estimate diffraction grating efficiency based on groove density, blaze angle, and wavelength.</p>

      <div className="grid gap-4 sm:grid-cols-3 mb-8">
        <label className="block">
          <span className="text-gray-300 text-sm">Grooves/mm</span>
          <input type="number" value={groovesPerMm} onChange={e => setGroovesPerMm(+e.target.value)} min={50} max={6000}
            className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" />
        </label>
        <label className="block">
          <span className="text-gray-300 text-sm">Blaze Angle (°)</span>
