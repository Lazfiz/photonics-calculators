...
export default function CoherentAntiStokesPage() {
  ...
  const chartData = useMemo(() => {
    ...
    const carsIntensity = shifts.map(s => {
      const chiR = R / (s * 100 - vibFreq / 100 / (2 * Math.PI * c / 100) + 1i * gamma);
      const chi = A_NR + chiR;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Coherent Anti-Stokes Raman Spectroscopy (CARS)</h1>
      <p className="text-gray-400 mb-8">Four-wave mixing process: ω_CARS = ω_pump − ω_Stokes + ω_probe. Coherent, directional signal above fluorescence.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block">
          <span className="text-gray-300 text-sm">Stokes Wavelength (nm)</span>
      ...
      <div className="bg-gray-900 rounded-lg p-4 mb-6">
        ...
        <p className="text-gray-300 text-sm"><span className="text-green-400">Vibrational frequency:</span> {vibWavenumber.toFixed(1)} cm⁻¹ ({(vibFreqHz/1e12).toFixed(2)} THz)</p>
        <p className="text-gray-300 text-sm"><span className="text-green-400">CARS wavelength:</span> {CARSnm.toFixed(2)} nm</p>
        <p className="text-gray-300 text-sm"><span className="text-green-400">Thermal factor n(Ω):</span> {thermalFactor.toFixed(4)}</p>
      </div>
