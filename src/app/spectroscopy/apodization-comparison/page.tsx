...
export default function ApodizationComparisonPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-5xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Apodization Comparison</h1>
      <p className="text-gray-400 mb-8">Compare 9 window functions and their instrument line shapes (ILS). Select windows to overlay.</p>

      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">N Points</span>
          <input type="number" value={nPoints} onChange={e => setNPoints(Math.max(16, +e.target.value))} min={16} max={2048} step={64} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">View Range (dB)</span>
          <input type="number" value={viewDb} onChange={e => setViewDb(+e.target.value)} min={-120} max={-10} step={-5} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
