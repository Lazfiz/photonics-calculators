...
export default function NarrowBandpassPage() {
  ...
  const [mirrorPairs, setMirrorPairs] = useState(6);
  const [cavities, setCavities] = useState(3);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Narrow Bandpass Filter</h1>
      <p className="text-gray-400 mb-8">High-finesse Fabry-Perot with multiple cavities for ultra-narrow transmission peaks.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Mirror Pairs</span>
          <input type="number" value={mirrorPairs} onChange={e => setMirrorPairs(+e.target.value)} min={2} max={20} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4">
        ...
        <p className="text-gray-400 text-sm font-mono">d<sub>QW</sub> = λ₀/(4n) — quarter-wave mirror layers</p>
        <p className="text-gray-400 text-sm font-mono">d<sub>cavity</sub> = λ₀/(2n<sub>sp</sub>) — half-wave spacer</p>
        <p className="text-gray-400 text-sm font-mono">F = π√R/(1-R) — mirror finesse</p>
        <p className="text-gray-400 text-sm font-mono">FWHM = λ₀/(m·F) — m = cavity order</p>
        <p className="text-gray-400 text-sm font-mono">F<sub>eff</sub> ≈ F·(N<sub>cav</sub>)^2 — multi-cavity sharpening</p>
      </div>
