...
export default function IonAssistedDepositionPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Ion-Assisted Deposition (IAD)</h1>
      <p className="text-gray-400 mb-8">Design ion beam parameters for improved packing density and stress control. Models ion-to-atom ratio, packing density, and compressive stress.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Ion Energy (eV)</span>
          <input type="number" value={ionEnergy} onChange={e => setIonEnergy(+e.target.value)} step="10" min="0" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Ion Current (mA)</span>
          <input type="number" value={ionCurrent} onChange={e => setIonCurrent(+e.target.value)} step="1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Deposition Rate (nm/s)</span>
          <input type="number" value={depositionRate} onChange={e => setDepositionRate(+e.target.value)} step="0.1" min="0.01" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Ion Mass (amu)</span>
          <input type="number" value={ionMass} onChange={e => setIonMass(+e.target.value)} step="1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Chamber Pressure (Torr)</span>
          <input type="number" value={chamberPressure} onChange={e => setChamberPressure(+e.target.value)} step="1e-5" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
