...
export default function NotchFilterPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Notch Filter</h1>
      <p className="text-gray-400 mb-8">Rejection notch filter — high reflectance at target wavelength, transmits elsewhere.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Notch λ (nm)</span>
          <input type="number" value={notchWl} onChange={e => setNotchWl(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Mirror Pairs</span>
          <input type="number" value={mirrorPairs} onChange={e => setMirrorPairs(+e.target.value)} min={1} max={15} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Cavity n</span>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4">
        ...
        <p className="text-gray-400 text-sm font-mono">d<sub>H,L</sub> = λ₀/(4n) — quarter-wave mirrors</p>
        <p className="text-gray-400 text-sm font-mono">d<sub>cavity</sub> = λ₀/(4n<sub>cavity</sub>) — quarter-wave cavity</p>
        <p className="text-gray-400 text-sm font-mono">Notch depth → R<sub>max</sub> via mirror reflectance &amp; cavity coupling</p>
      </div>
