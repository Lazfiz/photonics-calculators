...
export default function HardCoatingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Hard Coating Design</h1>
      <p className="text-gray-400 mb-8">Abrasion-resistant optical coating — balance mechanical hardness with optical performance.</p>

      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">n<sub>coating</sub></span>
          <input type="number" value={nCoat} onChange={e => setNCoat(+e.target.value)} step="0.01" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        ...
        <label className="block"><span className="text-gray-300 text-sm">Hardness (HK₀.₀₁)</span>
          <input type="number" value={hardness} onChange={e => setHardness(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Stress (GPa, negative=tension)</span>
          <input type="number" value={stressGPa} onChange={e => setStressGPa(+e.target.value)} step="0.1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4">
        ...
        <p className="text-gray-400 text-sm font-mono">d<sub>QW</sub> = λ₀/(4n) — quarter-wave AR condition</p>
        <p className="text-gray-400 text-sm font-mono">R<sub>min</sub> = [(n<sub>sub</sub>-n<sub>coat</sub>²)/(n<sub>sub</sub>+n<sub>coat</sub>²)]² — zero-R when n<sub>coat</sub>=√n<sub>sub</sub></p>
        <p className="text-gray-400 text-sm font-mono">σ<sub>critical</sub> = √(2ΓE/πt) — delamination threshold</p>
        <p className="text-gray-400 text-sm font-mono">H/E ratio → wear resistance indicator</p>
      </div>
