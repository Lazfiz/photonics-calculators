...
export default function FabryPerotFilterPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Fabry-Pérot Filter</h1>
      <p className="text-gray-400 mb-8">Fabry-Pérot etalon/filter transmission based on the Airy function. Explore how mirror reflectance and cavity spacing control spectral selectivity.</p>

      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <label className="block"><span className="text-gray-300 text-sm">n (cavity)</span>
          <input type="number" value={nCavity} onChange={e => setNCavity(+e.target.value)} step="0.01" min="0.1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Cavity Spacing (nm)</span>
          <input type="number" value={spacing} onChange={e => setSpacing(+e.target.value)} step="10" min="1" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Mirror Reflectance</span>
          <input type="number" value={reflectance} onChange={e => setReflectance(+e.target.value)} step="0.01" min="0" max="0.999" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">Center Wavelength (nm)</span>
          <input type="number" value={wlCenter} onChange={e => setWlCenter(+e.target.value)} step="10" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      </div>
