...
export default function ProtectedSilverPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-2">Protected Silver Mirror</h1>
      <p className="text-gray-400 mb-8">Protected silver coating — high reflectance UV-Vis-IR with dielectric overcoat and adhesion layer.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        ...
        <label className="block"><span className="text-gray-300 text-sm">Ag Thickness (nm)</span>
          <input type="number" value={agThickness} onChange={e => setAgThickness(+e.target.value)} className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
        <label className="block"><span className="text-gray-300 text-sm">n<sub>protect</sub> (overcoat)</span>
        ...
        <label className="block"><span className="text-gray-300 text-sm">n<sub>adhesion</sub></span>
          <input type="number" value={nAdhesion} onChange={e => setNAdhesion(+e.target.value)} step="0.01" className="mt-1 w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white" /></label>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-4">
        ...
        <p className="text-gray-400 text-sm font-mono">Ag: Drude model — ε(ω) = ε∞ − ωₚ²/(ω² + iγω)</p>
        <p className="text-gray-400 text-sm font-mono">TMM with complex n + ik for metallic layer</p>
        <p className="text-gray-400 text-sm font-mono">R = |(n₁−n₂)² + k²| / |(n₁+n₂)² + k²| (single interface)</p>
      </div>
