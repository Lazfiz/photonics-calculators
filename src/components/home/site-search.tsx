  ...
  return new Promise((resolve) => {
    ...
    script.onload = async () => {
      try {
        ...
        if (!hf?.pipeline) { resolve(null); return; }
...
export default function SiteSearch() {
  ...
  const ensureModel = useCallback(async () => {
    ...
    try {
      const [embs] = await Promise.all([loadEmbeddings(), getEmbedFn()]);
      if (!embs) { setModelStatus("error"); return; }
      if (!embedFn) { setModelStatus("error"); return; }
  ...
  useEffect(() => {
    ...
    // Semantic search for longer queries
    if (q.length < 10) return;
    debounceRef.current = setTimeout(async () => {
      if (modelStatus === "idle") await ensureModel();
      if (modelStatus !== "ready" || !embedFn) return;
      try {
        const queryEmb = await embedFn(q);
        if (!queryEmb) return;
        const embs = await loadEmbeddings();
        const kwSet = new Set(kw.map((i) => i.href));
        const semResults = items
          .filter((item) => !kwSet.has(item.href))
          .map((item) => {
            const emb = embs.find((e) => e.href === item.href);
            if (!emb) return { item, sim: 0 };
            return { item, sim: cosineSimilarity(queryEmb, emb.embedding) };
          })
          .filter(({ sim }) => sim > 0.3)
          .sort((a, b) => b.sim - a.sim)
          .map(({ item }) => item);
        setResults([...kw, ...semResults].slice(0, 12));
      } catch { /* keyword results still showing */ }
    }, 600);
